import { seedInventoryCategories, seedInventoryItems, seedPackingContainers, seedPhases } from "./seed";
import type { BuyLocation, InventoryCategory, InventoryItem, InventoryPriority, InventoryStatus, PackingContainer, Phase, RelocationTask, TaskStatus } from "./types";
...
export const flattenData = (phases: Phase[], inventoryCategories: InventoryCategory[] = seedInventoryCategories, inventoryItems: InventoryItem[] = seedInventoryItems, packingContainers: PackingContainer[] = seedPackingContainers): RelocationStorageData => ({
  ...
  inventoryCategories,
  inventoryItems: inventoryItems.map((item, index) => ({ ...item, packingOrder: item.packingOrder ?? index })),
  packingContainers,
...
const seedData = () => flattenData(seedPhases, seedInventoryCategories, seedInventoryItems, seedPackingContainers);
...
const validateInventoryItem = (item: unknown, categoryIds: Set<string>, containerIds: Set<string>): item is InventoryItem => {
...
const withStoredOrder = (data: RelocationStorageData): RelocationStorageData => ({
  ...
  inventoryItems: data.inventoryItems.map((item, index) => ({ ...item, packingOrder: item.packingOrder ?? index })),
});
...
const migrateV1Data = (value: Record<string, unknown>): RelocationStorageData | null => {
  ...
  return {
    ...
    inventoryItems: seedInventoryItems.map((item, index) => ({ ...item, packingOrder: item.packingOrder ?? index })),
...
export const validateData = (value: unknown): { ok: true; data: RelocationStorageData } | { ok: false; error: string } => {
  ...
  if (!value.inventoryItems.every((item) => validateInventoryItem(item, categoryIds, containerIds))) return { ok: false, error: "One or more inventory items are invalid." };
...
export const exportData = (phases: Phase[], inventoryCategories?: InventoryCategory[], inventoryItems?: InventoryItem[], packingContainers?: PackingContainer[]) => JSON.stringify(flattenData(phases, inventoryCategories, inventoryItems, packingContainers), null, 2);
